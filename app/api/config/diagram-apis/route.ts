...
export async function GET() {
  try {
    const stmt = db.prepare('SELECT * FROM diagram_apis ORDER BY priority DESC, id ASC');
    const apis = stmt.all();
    ...
    console.error('获取绘图API配置失败:', error);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    const stmt = db.prepare(`
      INSERT INTO diagram_apis (name, api_endpoint, api_key, is_active, priority)
      VALUES (?, ?, ?, ?, ?)
    `);
    ...
    console.error('添加绘图API配置失败:', error);
    return NextResponse.json(
